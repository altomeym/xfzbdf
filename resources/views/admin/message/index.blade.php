          ...
          <div class="box-header with-border">
            ...
            <div class="box-tools col-lg-4 pull-right nopadding-right">
              <div class="has-feedback">
                {!! Form::open(['route' => 'search.message', 'method' => 'get', 'id' => 'form', 'data-toggle' => 'validator']) !!}
            ...
            <div class="mailbox-controls">
              ...
              <div class="btn-group ">
                ...
                <ul class="dropdown-menu" role="menu">
                  <li><a href="javascript:void(0)" data-link="{{ route('admin.support.message.massUpdate', [App\Models\Message::STATUS_NEW, 'status']) }}" class="massAction" data-doafter="reload">
                    ...
                    <tr id="item_{{ $message->id }}">
                      ...
                      <td class="mailbox-name">
                        <a href="{{ route('admin.support.message.show', $message) }}">
                          @if ($message->isUnread())
                            <?php // hassan $message->customer->getName() replaced by $message->name ? $message->name : $message->customer->getName()?>
                            <strong>{!! highlightWords($message->name ? $message->name : $message->customer->getName(), $search_q) !!}</strong>
                          @else
                      ...
                      <td class="mailbox-subject">
                        <a href="{{ route('admin.support.message.show', $message) }}" style="{{ $message->isUnread() ? 'color: #222;' : '' }}">
                          <strong>{!! highlightWords($message->subject, $search_q) !!} </strong> - {!! highlightWords(Str::limit(strip_tags($message->message), 180 - strlen($message->subject)), $search_q) !!}
                        </a>
                      </td>
                      <td class="">
                        <small>
