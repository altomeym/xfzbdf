      ...
      <div class="form-group has-feedback">
        ...
        <select name="country_id" class="form-control" required>
          @foreach(get_countries() as $country)
            <option value="{{ $country->id }}" @if(@$geoip['iso_code'] == $country->iso_code) selected @endif>{{ $country->name }}</option>
          @endforeach
        </select>
        <span class="glyphicon glyphicon-globe form-control-feedback" style="margin-top:-5px"></span>
        <div class="help-block with-errors"></div>
      </div>
