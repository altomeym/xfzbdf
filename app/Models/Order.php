...
class Order extends BaseModel
{
    ...
    {
        // added by hassan00942
        $this->attributes['billing_address'] = '';
        // commented by hassan00942
    ...
    public function invoice($des = 'D')
    {
        ...
        // array_push($invoiceTo, $this->customer->name);
        ...
        // added by hassan00942
        $invoice->setCustomerId($this->customer->id);   // Reference
        ...
        if (App::environment('production') && Storage::exists($logo_path)) {
            // commented by hassan00942
            // $invoice->setLogo(get_storage_file_url($logo_path, null), null);
            // added by hassan00942
            $invoice->setLogo(get_storage_file_url($logo_path, null), 190, 90);
        }

        $invoice->setReference($this->order_number);   // Reference
        $invoice->setDate($this->created_at->format('M d, Y'));   //Billing Date
        // 1 line commented by hassan00942
        ...
        // start qr code setting by hassan00942 + to save generate qr code
        // return $this->id;
        $image = \SimpleSoftwareIO\QrCode\Facades\QrCode::format('png')
                //  ->merge('img/t.jpg', 0.1, true)
                 ->size(200)->errorCorrection('H')
                 ->generate(route('order.invoice', $this));
        $save_location = "public/qr-code/{$this->customer->id}-{$this->id}.png";
        $qr_image_url =  url("image/qr-code/{$this->customer->id}-{$this->id}.png");
        Storage::disk('local')->put($save_location, $image);
        $invoice->setQrImageUrl($qr_image_url);
        // return $qr_image_url;
        // end qr code setting by hassan00942 + to save generate qr code
        ...
        $invoice->addPaymentStatusName($this->paymentStatusName(true));
